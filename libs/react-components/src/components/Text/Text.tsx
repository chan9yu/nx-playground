...
const Text: TextComponent = forwardRef<HTMLElement, TextProps>((props, ref) => {
	const { as = 'p', color = 'gray', background, children, fontSize = 'md', ...rest } = props;

	const childSprinkleProps = extractSprinkleProps(props, Array.from(StyleSprinkles.properties));
	const className = clsx([BaseStyle, StyleSprinkles(childSprinkleProps), textStyle({ fontSize }), rest.className]);
